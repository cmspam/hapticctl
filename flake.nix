  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      homeManagerModules.default = { config, lib, pkgs, ... }:
        ...
        {
          options.services.hapticctl = {
            enable = lib.mkEnableOption "hapticctl haptic touchpad intensity restore (user service)";
            ...
            defaultIntensity = lib.mkOption {
              type = lib.types.int;
              default = 50;
              description = "Default intensity if no saved state exists (0-100).";
            };
          };

          config = lib.mkIf cfg.enable {
            home.packages = [ hapticctl ];

            systemd.user.services.hapticctl-restore = {
              Unit = {
                Description = "Restore haptic touchpad intensity";
              ...
              Service = {
                ...
                ExecStart =
                  let
                    deviceArg = lib.optionalString (cfg.device != null) "--device ${cfg.device}";
                  in
                  "${hapticctl}/bin/hapticctl ${deviceArg} restore";
              };
